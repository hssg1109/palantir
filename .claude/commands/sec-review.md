...
- **조치 불필요/감사기록성 finding → 오탐(양호) 처리, 리포팅 대상 아님 (2026-09-08 도입, 2026-09-14 일반화)**: 1~3단계 판정 결과 `review_result: "정보"`로 확정되었더라도, 최종 확정 전 아래 조건①을 만족하는지 확인한다 — 만족하면 `review_status: "오탐"`, `result: "양호"`로 재분류하고 보고서에서 제외한다 (severity 최저기준 적용 대상에서도 제외).
  - **조건① (필수)**: `description`/`recommendation`이 명시적으로 "조치 불필요"/"해당없음"/"○○ 관점에서는 성립하지 않음"으로 결론 나 있어, 리포트를 받는 개발팀이 이 finding 자체로는 실제로 취할 조치가 없음. `recommendation` 전체가 (수식어를 걷어내면) "조치 불필요"로만 귀결되고 그 외 잔존 실질 조치 항목이 전혀 없으면 조건①은 그 자체로 충족된다 — 아래 조건②는 조건①을 보강하는 **부가 근거**일 뿐 별도 필수 요건이 아니다(2026-09-14 일반화: 종전에는 조건①·②를 모두 요구했으나, 잔존 조치가 전혀 없는 순수 FP까지 "다른 정탐 finding으로 커버돼야만" 오탐 처리가 가능하다고 좁게 해석해 homeshopping/trend-ad/trend-cms/trendissue 4개 레포 PHP Weak Crypto·LFI 계열 7건이 "정보 — 조치 불필요합니다"라는 자기모순 문구로 리포팅되는 사고가 발생함).
  - **조건② (부가, 있으면 함께 기록)**: 그 근본 원인(대개 동일 파라미터/동일 입력 경로)이 이미 **다른 정탐 finding**(예: 같은 파라미터의 Persistent XSS)으로 완전히 커버되어, 이 finding이 남아 있어도 새로 드러나는 조치 항목이 없음(순수 중복). 조건②가 성립하면 review_note에 관련 정탐 finding_id를 함께 남긴다.
  - 조건①이 미충족(예: "HTTP 직렬화 경로 재확인 필요"/"코드리뷰 시 사용자 입력 유입 여부 확인" 등 확인 액션이 recommendation에 남아 있음)이면 이 규칙 적용 대상이 아니며 기존 §4 절차대로 정보/Medium으로 유지한다 — 이 경우 recommendation의 "조치 불필요" 표현은 제거하고 잔존 조치만 남기도록 재작성한다.
  - `review_note`에는 감사 추적을 위해 "무엇을 검토했는지 + 왜 해당없음인지 (+ 조건② 해당 시 관련 정탐 finding_id)"를 남긴다(예: `"View XSS 관점 41건 수동확인 완료, HTML 렌더링 경로 없어 해당없음 확인 — 동일 파라미터는 XSS-002(Persistent, 정탐)로 이미 커버되어 별도 조치 없음. 오탐 처리."`, 또는 조건②가 없는 경우 `"PHPExcel 벤더 라이브러리 내부 캐시 키 용도로만 사용, 인증/비밀번호 무관하며 잔존 조치 없음. 오탐 처리."`).
  ...
  - **근거 (2026-09-14 homeshopping 등 4개 레포 PHP Weak Crypto/LFI)**: 사용자가 "정보성 취약점의 조치요약에 '조치 불필요합니다'가 명시되면 안 됨 — 리포팅 대상이면서 조치 불필요라는 워딩이 공존하는 것은 모순"이라고 직접 지적. 조건②(다른 정탐 finding으로 커버)가 없어도, recommendation이 순수하게 조치 불필요로만 귀결되면 그 자체로 리포팅 대상이 아니라는 점을 반영해 조건①만으로 충분하도록 일반화. `sec-scan-php`의 `task_php_llm_review.md` §3/§6 판정표에도 동일 원칙을 소스 단계에 반영(벤더 라이브러리·완전 고정 경로는 "정보" 대신 즉시 "오탐"으로 판정하도록 개선).
...
**1단계-b — 비파일럿 카테고리 확장 후보 탐지 (병합 아님, 사용자 확인 트리거) ⚠️ 필수 — §1단계 직후 실행**

> **목적 (2026-09-11 oip_batch 재발 방지)**: §1단계의 기계적 스캔은 `HARDCODED_SECRET`/`DTO_EXPOSURE` 2개
> 파일럿 카테고리에서만 파일 간 병합 "후보"를 찾는다. 그 외 카테고리는 애초에 스캔 대상이 아니므로,
> 실제로는 같은 근본원인(예: 특정 프로파일의 로그레벨 설정 하나)으로 여러 파일에 반복 노출된 케이스가
> 있어도 §1단계는 이를 발견조차 하지 못하고 조용히 지나간다. `oip_batch`에서 DEBUG 레벨 민감정보
> 로깅(`SENSITIVE_LOGGING`, 파일럿 카테고리 아님) 4개 파일이 전부 "운영 opt 프로파일 rootLogger=DEBUG"
> 라는 동일 근본원인을 공유했는데, §5a 자동 스캔이 이를 탐지하지 못해 사용자가 리포트를 직접 읽고서야
> 지적한 사례가 발생했다. 이 단계는 그 탐지 공백을 메운다 — 병합을 자동 실행하지는 않되, 최소한
> **후보 존재 자체는 반드시 사용자에게 보고**한다.

1. `reviewed: true` + `review_status: "정탐"` + 카테고리가 `HARDCODED_SECRET`/`DTO_EXPOSURE`가 **아닌**
   나머지 finding 전체를 `category` + `severity` 기준으로 groupby한다 (파일 무관).
2. 그룹 내 finding이 2건 이상이고 서로 다른 파일에 걸쳐 있으면, 공통 신호가 있는지 확인한다 —
   판정 기준은 카테고리별로 다르다:
   - **`SENSITIVE_LOGGING`/`민감정보 로깅` (기존 확립된 관행 — 2026-09-11 corpus 조사로 확인)**:
     업무 로직/파일이 서로 달라도 **로그레벨 tier가 같으면 그 자체로 공통 신호로 간주**한다 — 텍스트
     상 근본원인 서술 일치를 요구하지 않는다. Tier는 2종: (a) "운영 레벨" — `System.out.println`,
     `logger.info`/`warn`/`error` 등 프로파일·로그레벨 설정과 무관하게 상시 활성인 레벨, (b) "개발/debug
     레벨" — `logger.debug` 등 통상 운영에서 비활성이나 해당 레포의 운영 프로파일에서 예외적으로 활성화된
     경우. `severity`가 같은 tier끼리만 그룹화하고(운영 tier와 debug tier는 severity가 갈리는 것이
     일반적이므로 자연히 분리됨), 레포당 최대 2개 그룹(운영/debug)으로 수렴하는 것이 정상이다. 선례:
     `oip_admin` DATA-ADMIN-001(5개 파일 5개 서로 다른 업무 로직을 "운영 레벨(INFO/System.out)" 하나로
     병합), `oip_batch` DATA-BATCH-002(운영 레벨 5건 병합)/DATA-BATCH-003(DEBUG 레벨 4건 병합).
   - **그 외 카테고리**: `severity_rationale`/`manual_review_note`/`review_note` 텍스트에서 근본원인을
     가리키는 공통 신호가 있는지 확인한다 — 예: 동일한 설정 파일 경로(`profile/opt/log4j.properties` 등)
     언급, 동일한 환경/프로파일명 반복 인용, 동일한 공통 헬퍼/유틸리티 클래스 인용, "동일한 원인으로",
     "동일 패턴이 반복" 류의 서술이 여러 finding에 걸쳐 나타나는 경우.
3. 공통 신호가 확인되는 그룹이 하나 이상이면(0건이면 이 단계 자체를 생략), §4/§5d와 달리 **이 지점은
   `AskUserQuestion` 사용이 허용되는 유일한 예외 지점이다** — 아래 형식으로 그룹별 후보를 한 번에
   제시하고 병합 여부(그룹 전체/그룹 일부/병합 안 함)를 확인한다:
   ```text
   [MERGE-CANDIDATE] 비파일럿 카테고리 cross-file 병합 후보 발견 — {N}개 그룹
     그룹1: category={category}, severity={severity}, {M}건 — {finding_id 목록}
            공통원인: {식별된 공통 신호 1문장}
     ...
   ```
4. 사용자가 병합을 승인한 그룹만 §3단계(병합 실행) 절차를 그대로 적용한다. 이때도 카테고리 확장은
   **해당 리뷰 세션·해당 그룹 한정**이며 파일럿 카테고리 목록 자체를 영구 확장하는 것이 아니다
   (파일럿 목록 자체를 넓히려면 별도로 사용자에게 명시적으로 확인).
5. 사용자가 거부하거나 응답이 "병합 안 함"이면 각 finding은 병합 없이 그대로 유지하고 다음 단계로 진행한다.
