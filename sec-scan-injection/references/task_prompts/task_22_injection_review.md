...
1. **먼저 요약본만 로드**: `state/<prefix>/injection_llm_summary.json` 을 읽는다.
   - 포함 내용: 전체 건수·유형별 통계, 원본 파일 경로, 카테고리별 대표 샘플 1~2건.
2. **상세 데이터가 필요할 때만 원본 접근**: `full_result_path` 에 명시된 경로를 `FileRead` 도구로 열되, **전체 파일을 한 번에 읽지 않는다**.
   - 특정 인덱스의 항목이 필요하면 `offset`/`limit` 파라미터로 해당 범위만 읽는다.
   - `diagnoses[N]` 한 건을 확인하려면: 파일에서 해당 JSON 배열 요소 위치를 추정한 뒤 부분 읽기(약 30~50줄)로 조회한다.
3. **탐색 우선순위**: `result: 취약` → `result: 정보 (taint 추적 실패)` → `needs_review: true` 순으로 처리한다.
...
d = json.load(open('state/<prefix>/injection.json'))
eps = d.get('endpoint_diagnoses', [])
info = [e for e in eps if e.get('result')=='정보' or e.get('needs_review')]
...
> ⚠️ **구조 경고 — 반드시 준수**: `group_judgments`는 **반드시 `sqli_endpoint_review` 키 안에 중첩**해야 한다.
> 최상위에 직접 `"group_judgments": [...]`를 쓰면 보고서 생성기가 LLM 판정을 인식하지 못하고
> "SQL 인젝션 수동검토 필요" 텍스트가 그대로 보고서에 출력된다. 아래 구조를 엄격히 따를 것.
>
> 올바른 구조:
> ```json
> { "sqli_endpoint_review": { "group_judgments": [...] } }   ✅
> ```
> 잘못된 구조:
> ```json
> { "group_judgments": [...] }                                ❌ (최상위 직접 기재 금지)
> ```
