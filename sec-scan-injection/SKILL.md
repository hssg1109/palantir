...
```bash
# Step 2-0: API 엔드포인트 목록 추출 (injection 스캔 필수 선행 작업)
python3 shared/scripts/scan_api.py <src> \
    -o state/<prefix>/api_scan.json

# Step 2-2: 인젝션 스캔 (API 인벤토리 필수)
...
Phase 3 완료 후 `state/<prefix>/summary_injection.md` 를 생성한다.
...
| ID | 제목 | 심각도 | 파일:라인 | 판정 | 진단방법 |
|---|---|---|---|---|---|
| INJ-001 | SQL Injection — OrderBy 동적 파라미터 | High | src/.../Repo.java:120 | 취약 | 교차검증(수동) |
