...
Phase 3: LLM 수동분석 보완 (자동스캔 결과를 보완·갱신)
  ...
  │   │   ├─ 잠재적위협(Persistent XSS): ⚠️ REST_JSON이어도 양호 처리 금지 — DB write 직접 확인 필수
  │   │   │   ├─ call_chain에서 Repository.save()/insert()/update() 경로 확인
  │   │   │   ├─ 확인+String 필드 → 실제위협 (취약 확정)
  │   │   │   ├─ 확인+Integer/Enum 필드 → 양호 (FP)
  │   │   │   ├─ DB write 없음 확정(FeignClient/조회only) → 양호 (FP)
  │   │   │   └─ Kafka/MQ 경유만 → 잠재적위협 유지 (async taint break)
  ...
  │   │   └─ 판정 결과 → xss_endpoint_review 블록 저장 (xss_category_after_review 필드 포함)
  ...
  │   ⚠️ 진입 시 task_25_data_protection.md 재독 필수 (RULE-1~4 HARD RULE 적용)
  │   └─ RULE-1: SENSITIVE_LOGGING 로그레벨 2버킷 병합 / RULE-2: HARDCODED_SECRET 파일단위 병합
  │      RULE-3: DTO_EXPOSURE 컨트롤러 직접 확인 / RULE-4: 시크릿 원문 마스킹("****")
...
Phase 4: 보고서 생성 + Confluence 게시 [필수]
  ...
  │
  ├─ [필수 Step 4-1] 보고서 생성 전 민감정보 마스킹 — generate_finding_report.py 실행 전 필수
  │   ⛔ 아래 절차를 순서대로 수행한다. 생략 시 고객사 내부 인프라 정보가 Confluence에 노출된다.
  │   │
  │   ├─ Step 4-1-A: redact.py 자동 실행 (1차 방어선)
  │   │   아래 명령을 실행하여 state JSON 파일에서 IP·계정·JWT 등을 일괄 마스킹한다:
  │   │   ```bash
  │   │   # task25 자동스캔 결과 마스킹
  │   │   python3 tools/scripts/redact.py state/<prefix>/task25.json
  │   │   # task25 LLM 보완 결과 마스킹
  │   │   python3 tools/scripts/redact.py state/<prefix>/task25_llm.json
  │   │   # injection / xss / task24 에도 IP가 포함될 수 있으면 동일 실행
  │   │   # python3 tools/scripts/redact.py state/<prefix>/injection.json
  │   │   ```
  │   │   redact.py가 처리하는 항목: IPv4 주소, JDBC URL, DB 계정, 비밀번호·키, JWT, 이메일, 전화번호
  │   │
  │   ├─ Step 4-1-B: 수동 검증 (2차 방어선) — task25_llm.json을 직접 열어 확인
  │   │   → HARDCODED_SECRET code_snippet: 실제 패스워드·키·JWT Secret 원문 → `[REDACTED]` 치환
  │   │   → **IPv4 주소** (10.x.x.x, 192.168.x.x 등 내부망 IP) → `[REDACTED_IP]` 치환
  │   │   → **DB 계정명** (username=admin 등) → `[REDACTED_ACCOUNT]` 치환
  │   │   → **JDBC URL** (jdbc:mysql://host:port/db) → `jdbc:mysql://[REDACTED_JDBC_URL]` 치환
  │   │   → 자동스캔이 `"**** (마스킹)"` 처리한 값: 원본 복원 절대 금지, 마스킹 상태 유지
  │   │   ※ 상세 기준: task_25_data_protection.md RULE-4 참조
  │   │
  │   └─ Step 4-1-C: 확인 완료 후 generate_finding_report.py 실행
  │       (code_snippet 원문이 보고서에 그대로 출력되므로 이 단계 생략 불가)
  │
...
> **📖 실행 전 필수**: Task 3-5 진입 즉시 `skills/sec-audit-static/references/task_prompts/task_25_data_protection.md` 를
> **반드시 다시 읽어라** — RULE-1(SENSITIVE_LOGGING 병합) / RULE-2(HARDCODED_SECRET 병합) /
> RULE-3(DTO_EXPOSURE 컨트롤러 직접 확인) / RULE-4(고객사 민감정보 마스킹)
> 이 4개 규칙은 아래 체크리스트와 함께 **모두 충족해야** task25_llm.json 출력이 완료된 것으로 인정된다.

> **⛔ HARD RULE — 아래 3가지 미충족 시 Task 3-5 미완료 처리 (task25_llm.json 출력 불가)**
>
> 1. **SENSITIVE_LOGGING 로그레벨 단위 병합** — 개별 건/모듈별 분리 출력 금지
> 2. **HARDCODED_SECRET 파일/환경 단위 병합** — 동일 파일 다건·동일 환경 동일유형 다건을 각 1건으로 통합
> 3. **DTO_EXPOSURE 실 취약 여부 확인** — affected_endpoints 기반 Controller 코드 직접 확인 필수
>
> **⛔ HARD RULE — 고객사 민감정보 마스킹 (finding 출력 전 필수 적용)**
>
> finding.evidence.code_snippet에 **실제 시크릿 원문 절대 포함 금지**:
> - HARDCODED_SECRET: 실제 비밀번호·API키·JWT Secret·DB패스워드 원문 → `"****"` 또는 `"[REDACTED]"` 치환
> - 자동스캔이 이미 `"**** (마스킹)"` 처리한 경우: 원본 값 복원 절대 금지, 마스킹 상태 그대로 유지
> - LLM이 Read 툴로 파일을 읽어 evidence 첨부 시: 시크릿 원문이 보이면 기재 전 마스킹 후 출력
> - 허용: 변수명, 파일경로, 라인번호, 컨텍스트 설명 (비민감 메타데이터)
...
□ [HARD RULE] SENSITIVE_LOGGING 병합: 로그 레벨 기준 2버킷 (모듈별/파일별 분리 금지)
...
□ [HARD RULE] HARDCODED_SECRET 파일/환경 단위 병합 (task_25 Step 5 기준 엄수)
  → 동일 파일 내 여러 라인 → 1개 finding (lines 배열에 전체 라인 나열)
  → 동일 환경의 동일 유형 파일(ccdev/*.properties 3파일 등) → 1개 finding
  → 환경별 severity: src/main/java(Critical) / resources/(High) / ccalp(High) / ccdev(Medium) / local(Low)
  → src/main/java/ 경로 = 운영 코드 → needs_review: false 강제 (별도 확인 불필요)

□ [HARD RULE] DTO_EXPOSURE: 스크립트 역추적 결과 + Controller 코드 직접 확인
  → task25.json DTO finding의 affected_endpoints 배열 확인
  → affected_endpoints 비어 있음 → INTERNAL/Consumer DTO → FP 처리
  → affected_endpoints 있음 → Controller 코드 직접 확인 후 판정:
       ├─ 본인 정보 반환 API (Safe by Design) → 양호
       ├─ @JsonIgnore/@JsonSerialize 적용 → 양호
       └─ PII 필드 노출 + 타인 조회 가능 → 취약 (finding 확정)
  → 한글 주석 파싱 버그(민감 필드(//), 민감 필드(용)) → FP 확인 필수

□ [HARD RULE] 민감정보 마스킹 — finding 출력 전 자가 검증
  → HARDCODED_SECRET evidence.code_snippet: 실제 시크릿 원문 → "****" 치환 필수
  → 자동스캔 "**** (마스킹)" 상태 → 원본 값 복원 절대 금지
  → LLM Read 툴 직접 읽기 후 evidence 작성 시: 시크릿 원문 발견 시 기재 전 마스킹 처리
  → 위반 시: task25_llm.json 저장 전 전체 code_snippet 재검토 후 재작성
