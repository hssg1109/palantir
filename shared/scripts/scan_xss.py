...
변경 이력:
  ...
  v2.7.0 - Fix 1: View XSS / Reflected XSS 상호 배타적 판정 (중복 카운트 방지)
           ...
           Fix 2: Open Redirect diagnosis_detail 오염 방지
            Phase 4 redirect severity override 시 View 파일 탐색 실패 메시지가
            diagnosis_detail에 병합되지 않도록 _redirect_prev 필터링 로직 추가
           Fix 3: DOM XSS 서드파티 제외 정규식 대폭 확장 (_P6_DOM_EXCLUDE_RE)
            에디터: daumeditor, smarteditor, ckeditor, tinymce, froala, quill, summernote
            플러그인 파일: printThis, smartpop, magnific-popup, fancybox, swfobject 등
            레거시: prototype, mootools, extjs
            디렉토리: /plugins/, /assets/js/, /ext/lib/, /static/js/plugin/ 등
...
_P6_DOM_EXCLUDE_RE = re.compile(
    ...
    r'(?:[.\-][\d.]+)?(?:\.min)?\.js$'
    # 에디터 라이브러리 (daumeditor, smarteditor, ckeditor, tinymce 등)
    r'|(?:daumeditor|smarteditor|ckeditor|tinymce|froala|quill|summernote|'
    r'codemirror|ace|monaco-editor|trumbowyg|prosemirror)'
    r'(?:[.\-][\d.]+)?(?:\.min)?\.js$'
    # 팝업·슬라이더·프린트 플러그인
    r'|(?:printThis|smartpop|magnific[-.]popup|fancybox|colorbox|'
    r'swfobject|flot|highcharts|leaflet|chart|d3)'
    r'(?:[.\-][\d.]+)?(?:\.min)?\.js$'
    # 기타 레거시 라이브러리
    r'|(?:prototype|mootools|ext[-.]all|extjs)'
    r'(?:[.\-][\d.]+)?(?:\.min)?\.js$'
    # 벤더·플러그인·에셋 디렉토리 경로 패턴
    r'|[\\/](?:lib|vendor|common[/\\]js|dist|bower_components|'
    r'node_modules|static[/\\]js[/\\]lib|webapp[/\\]js[/\\]lib)[\\/]'
    # 추가 서드파티 디렉토리: plugins, ext, assets, static/js/plugin, static/js/ext
    r'|[\\/]plugins?[\\/]'
    r'|[\\/](?:static|webapp|resources)[/\\]js[/\\](?:ext|plugin|plugins?|assets)[\\/]'
    r'|[\\/]assets[\\/](?:js|javascript|scripts?)[\\/]'
    r'|[\\/]ext[\\/](?:js|lib|dist)[\\/]'
    r')',
    re.IGNORECASE,
)
    ...
    if ct == "REST_HTML_RISK":
        ...
        if view_file and view_file.exists():
            ...
            if is_vuln:
                ...
                if filter_ok:
                    # HTML_VIEW: View XSS 판정 우선 — reflected_xss는 해당없음(중복 카운트 방지)
                    out["reflected_xss"] = "해당없음"
                    out["view_xss"]      = "정보"
                    out.update({
                        "result":   "정보",
                        "severity": "Low",
                        "xss_type": "View XSS (필터 완화)",
                        "diagnosis_detail": (
                ...
                # HTML_VIEW 안전 확인: view_xss 양호, reflected_xss 해당없음
                out["reflected_xss"] = "해당없음"
                out["view_xss"]      = "양호"
                out.update({
                    "result":   "양호",
        ...
        # unknown controller type — 어느 쪽인지 불명확하므로 정보 처리, 중복 카운트 방지를 위해
        # view_xss만 정보로 기록하고 reflected_xss는 해당없음으로 유지
        out["reflected_xss"] = "해당없음"
        out["view_xss"]      = "정보"
        out.update({
            "result":       "정보",
    ...
    if p4["has_redirect_risk"]:
        ...
        if _SEV_RANK.get(redirect_severity, 0) > _SEV_RANK.get(out["severity"], 0):
            ...
            _redirect_prev = (
                prev_detail
                if prev_detail
                and "View 파일" not in prev_detail
                and "view_name=" not in prev_detail
                and "탐지 실패" not in prev_detail
                else ""
            )
            out.update({
                "result":   redirect_verdict,
...
def _write_llm_summary(result: dict, output_path: Path) -> None:
    """Phase 2 완료 후 LLM용 초경량 요약본을 _llm_summary.json으로 저장."""
    summary = result.get("summary", {})
    diagnoses = result.get("endpoint_diagnoses", [])

    # xss_category + xss_type 기준으로 샘플 최대 2건 추출
    samples: dict = {}
    for d in diagnoses:
        if d.get("result") not in ("취약", "정보"):
            continue
        cat = d.get("xss_category") or d.get("xss_type") or d.get("result", "기타")
        if cat not in samples:
            samples[cat] = []
        if len(samples[cat]) < 2:
            samples[cat].append({
                "endpoint": f"{d.get('http_method', '')} {d.get('request_mapping', '')}",
                "result": d.get("result", ""),
                "xss_type": (d.get("xss_type") or "")[:80],
                "brief": (d.get("diagnosis_detail") or "")[:120],
            })

    llm_summary = {
        "scan_type": "xss",
        "generated_at": result.get("scan_metadata", {}).get("scanned_at", ""),
        "full_result_path": str(output_path.resolve()),
        "statistics": summary,
        "samples_by_category": samples,
    }

    summary_path = output_path.with_name(output_path.stem + "_llm_summary.json")
    summary_path.write_text(json.dumps(llm_summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main():
    ...
    print(f"\n결과 저장: {out_path}")
    _write_llm_summary(result, out_path)
