...
def _write_llm_summary(output: dict, out_path: Path) -> None:
    """Phase 2 완료 후 LLM용 초경량 요약본을 _llm_summary.json으로 저장."""
    summary = output.get("summary", {})
    meta = output.get("scan_metadata", {})

    # 취약/info 항목에서 각 카테고리별 샘플 최대 2건 추출
    samples: dict = {}
    for category, items in [
        ("upload", output.get("upload_diagnoses", [])),
        ("download", output.get("download_diagnoses", [])),
        ("rfi", output.get("rfi_diagnoses", [])),
    ]:
        vuln_items = [i for i in items if i.get("result") in ("vulnerable", "info")]
        if vuln_items:
            samples[category] = [
                {
                    "file": i.get("controller_file", i.get("file", "")),
                    "result": i.get("result", ""),
                    "severity": i.get("severity", ""),
                    "brief": (i.get("detail") or "")[:120],
                }
                for i in vuln_items[:2]
            ]

    llm_summary = {
        "scan_type": "file_processing",
