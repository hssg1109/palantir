#!/usr/bin/env python3
"""
scan_data_protection.py v1.4.0
================================================================================
...
# ── 민감정보 마스킹 패턴 (snippet 출력 전 치환) ────────────────────────────────
# 보고서·JSON에 고객사 내부 인프라 정보(IP, 계정 등)가 노출되지 않도록 한다.
_REDACT_SECRET_VALUE_RE = re.compile(
    # key = "value" 또는 key: value 형태에서 값 부분 추출
    r'(?i)(?P<key>(?:password|passwd|pwd|secret|token|apikey|api[_-]key|'
    r'accesskey|access[_-]key|secretkey|secret[_-]key|client[_-]secret|'
    r'private[_-]key|signing[_-]key|hmac[_-]key|auth[_-]key|'
    r'username|user[_-]name|account|user|account[_-]id))'
    r'\s*(?:[=:]+)\s*'
    r'(?P<quote>["\']?)(?P<value>[^\s"\']{4,})(?P=quote)',
)
_REDACT_IPV4_RE = re.compile(
    # IPv4 주소 (버전 번호 1.0.0 / 2.3.1 등 오탐 방지: 4옥텟 모두 필수 + 범위 제한)
    r'\b(?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)\.){3}'
    r'(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)\b',
)
_REDACT_JDBC_RE = re.compile(
...
def _masked_snippet(content: str, pos: int, window: int = 120) -> str:
    """_snippet과 동일하나 출력 전 IP·계정·시크릿 값 마스킹 적용.

    고객사 내부 인프라 정보(내부 IP, DB 계정, 시크릿 원문)가
    보고서나 Confluence에 그대로 노출되지 않도록 한다.
    변수명·파일경로·라인번호 등 메타데이터는 마스킹하지 않는다.
    """
    raw = _snippet(content, pos, window)
    # 1) JDBC URL → jdbc:mysql://[REDACTED_HOST]/[REDACTED_DB]
    raw = _REDACT_JDBC_RE.sub(lambda m: m.group(1) + "[REDACTED_JDBC_URL]", raw)
    # 2) key=value 패턴에서 값 부분 마스킹
    ...
    for fp in _iter_sources(source_dir):
        ...
        for m in _S_JWT_SECRET_RE.finditer(content):
            ...
            _add("HARDCODED_SECRET", severity,
                 "JWT Secret Key 하드코딩",
                 desc, rel, ln, _masked_snippet(content, m.start()),
                 "CWE-798", "A02:2021 Cryptographic Failures",
...
def _write_llm_summary(result_dict: dict, out_path: Path) -> None:
    """Phase 2 완료 후 LLM용 초경량 요약본을 _llm_summary.json으로 저장."""
    summary = result_dict.get("summary", {})
    findings = result_dict.get("findings", [])

    # category별 샘플 최대 2건 추출 (취약/정보만)
    samples: dict = {}
    for f in findings:
        if f.get("result") not in ("취약", "정보"):
            continue
        cat = f.get("category", "기타")
        ...
        if len(samples[cat]) < 2:
            samples[cat].append({
                ...
                "brief": (f.get("description") or "")[:120],
            })

    llm_summary = {
        "scan_type": "data_protection",
        "generated_at": result_dict.get("scanned_at", ""),
